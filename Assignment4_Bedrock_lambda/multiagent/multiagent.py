...
grounded in UAT documents stored under ``uat_documents``.
"""
...
def _load_chunks() -> list[dict]:
    ...
    for document_path in supported_documents:
        if document_path.suffix.lower() == ".pdf":
            loaded_documents = PyPDFLoader(str(document_path)).load()
        else:
            loaded_documents = TextLoader(str(document_path), encoding="utf-8").load()
    ...
    logger.info("Loaded %s UAT chunks from %s", len(chunks), DOCUMENT_DIRECTORY)
    return chunks
...
retrieval_agent = create_agent(
    ...
    system_prompt=(
        "You are a UAT document retrieval specialist. Always search the UAT documents before "
...
coverage_agent = create_agent(
    ...
    system_prompt=(
        "You are a senior UAT test analyst. Search the UAT documents, then evaluate scenario "
...
defect_agent = create_agent(
    ...
    system_prompt=(
        "You are a UAT defect triage specialist. Search the UAT documents, then assess severity, "
...
supervisor_agent = create_agent(
    ...
    system_prompt=(
        "You are the main UAT RAG supervisor. Answer only from the UAT document corpus. "
...
def invoke(payload, context=None):
    ...
    if not UAT_CHUNKS:
        return {"error": f"No supported documents were found under {DOCUMENT_DIRECTORY}."}
